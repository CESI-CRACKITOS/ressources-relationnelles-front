<template>
  <div class="flex h-screen overflow-scroll flex-col p-5 gap-2">
    <div v-for="user in users" :key="user.id" class="flex justify-between items-center border-b py-2">
      ...
      <a :href="`/profile/${user.id}`" class="flex items-center justify-center border border-indigo-600 px-4 py-2 text-indigo-600
        w-fit hover:bg-indigo-600 hover:text-white hover:cursor-pointer">
