<template>
  <div
    ...
    <div class="flex gap-2 max-xl:items-center flex-col w-full">
      ...
      <ButtonLink -link-to="/feed2" -button-label="Explorer" icon="fa-solid fa-magnifying-glass"></ButtonLink>
      <ButtonLink -link-to="/notifications" -button-label="Notifications" icon="fa-solid fa-bell"></ButtonLink>
      <ButtonLink :-link-to="'/profile/' + user.id" -button-label="Profile" icon="fa-solid fa-user">
      </ButtonLink>

      <button class="py-4 max-xl:w-12 rounded-full hover:bg-blue-300 bg-blue-200" @click="openModal">
    ...
    <router-link :to="'/profile/' + user.id"
      ...
      <div class="max-xl:hidden">
        <IconButtonComponent action="" :context-id="user.id" active="false" icon="fa-solid fa-ellipsis">
        </IconButtonComponent>
      </div>
    </router-link>
  </div>
