<template>
  <div class="flex flex-col gap-5 w-full">
    <input
      ...
      class="border-indigo-600 border-2 p-2 rounded-md"
      placeholder="Titre"
      autocomplete="off"
    />
    <div class="flex flex-col items-center gap-4 md:flex-row">
      <div class="flex flex-col gap-2">
        ...
        <select
          class="border-indigo-600 border-2 text-indigo-600 rounded-md p-2 w-80"
          v-model="inputData.categoryId"
          required
        >
      ...
      <div class="flex flex-col gap-2">
        ...
        <select
          class="border-indigo-600 border-2 text-indigo-600 rounded-md p-2 w-80"
          v-model="inputData.resourceTypeId"
          required
        >
        ...
        >
          <div class="py-1 w-full">
            <div
              ...
              :key="relationType.id"
              class="flex items-center gap-2 px-2 w-full"
            >
              <input
                type="checkbox"
                @change="handleRelation(relationType.id)"
                :checked="inputData.relationTypeIds.includes(relationType.id)"
              />
              {{ relationType.name }}
            </div>
          </div>
    ...
    <textarea
      ...
      class="border-2 border-indigo-600 p-2 rounded-lg"
    ...
    <div v-show="contentOptionsShown" class="flex gap-4 items-center pt-3">
      <ContentButton
        ...
        @add="addContent('application/pdf')"
      />
      <ContentButton
        icon="fa-image"
        class="py-2 px-4 text-lg rounded-full"
        @add="addContent('image/*')"
      />
    </div>

    <div>
      <div
        v-for="(content, index) in inputData.contents"
        :key="index"
      ...
      >
        <div class="input-wrapper" v-if="!content.id">
          <input
            ...
            @change="handleUpload($event.target.files[0], content)"
            class="hidden-input"
          />
    ...
    <div class="flex flex-col gap-1">
      ...
      <button-component
        @click="contentOptionsShown = true"
        v-show="allInputsFilled && !contentOptionsShown"
        >Ajouter un contenu</button-component
      >
    </div>
...
const allInputsFilled = computed(
  () =>
    ...
    inputData.value.contents.every((content) => content?.value || content.id)
)
...
const deleteContent = (eventContent, isInDb) => {
  inputData.value.contents = inputData.value.contents.filter(
    (content) => content.id !== eventContent.id
  )
  if (isInDb) {
...
const setModal = () => {
  inputData.value = {
    ...
    relationTypeIds: props.resource
      ? props.resource.relationTypes.map((relationType) => relationType.id)
      : [],
    toDelete: []
  }
}
